...
class Nvwa_enchane_SimVP(nn.Module):
    def __init__(self, shape_in, hid_S=16, hid_T=256, N_S=4, N_T=8, incep_ker=[3, 5, 7, 11], groups=8, args=None):
        super(Nvwa_enchane_SimVP, self).__init__()
        T, C, H, W = shape_in
        self.args = args
        print("###", shape_in)
    ...
    def forward(self, x_raw):
        ...
        # # Check if input needs padding to match T
        # if C < T:
        #     padding = T - C
        #     x_raw = torch.cat([x_raw] * padding, dim=1)  # Repeat frames to match T

        print("####x_raw.shape", x_raw.shape)
        print("####x_raw.size()", x_raw.size())
        x = x_raw.view(B * T, C, H, W)  # Flatten temporal dimension
...
if __name__ == "__main__":
    model = Nvwa_enchane_SimVP((4, 2, 32, 32))
    inputs = torch.rand(4, 4, 2, 32, 32)
    outputs = model(inputs)
    print(outputs.shape)
